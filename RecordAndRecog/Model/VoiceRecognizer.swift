...
final class VoiceRecognizer: NSObject {
    ...
    fileprivate let speechRecognizer = SFSpeechRecognizer(locale: Locale(identifier: "ko-KR"))!
...
private extension VoiceRecognizer {
    ...
    func recognize(ofIndex index: Int = 0) {
        ...
        //let audioPath = getFullPath(forFilename: <#T##String#>)
 
        
        recognitionRequest = SFSpeechURLRecognitionRequest(url: URL(fileURLWithPath: audioPath))
        
        //recognitionRequest = SFSpeechURLRecognitionRequest(url: getFullPath(forFilename: "audioFile.acc") )
