...
extension RecordedListVC:Presentable {
    enum Presenter: String {
        case cell = "RecordedFilePlayerSegue"
    }
    
    var destination: Presenter {
        return .cell
    }
    
    func display(_ destination: String?) {
        if let dest = destination {
            performSegue(withIdentifier: dest, sender: self)
        }
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == Presenter.cell.rawValue {
            guard let vc = segue.destination as? RecordPlayViewController else {
                return
            }
            
            vc.fileName = self.selectedFileName
        }
    }
}
...
//    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell: RecordCell = tableView.dequeueReusableCell(for: indexPath)
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        guard let cell: RecordCell = tableView.cellForRow(at: indexPath) as? RecordCell,let text = cell.fileNameLabel.text else {
           selectedFileName = ""
            return
        }
        
        selectedFileName = text
        display(Presenter.cell.rawValue)
    }
